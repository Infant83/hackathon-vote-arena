...
10. 관리자는 `/wall`의 `퀴즈` 모드에서 문제를 출제하고, 관객 답변과 선착순 정답자를 송출할 수 있습니다.
    최초 정답이 도착하면 3초 동안 `정답 확인 중...` 상태로 두되, 이 시간에도 다른 참가자의 답변 제출은 계속 받습니다.
    확인 시간이 끝나면 보정 제출시각 기준으로 최종 정답자를 확정합니다.
11. 관리자는 팀 정보와 안내 문구를 화면에서 편집하고, JSON/ZIP으로 가져오거나 내보낼 수 있습니다.
12. 관리자는 행사 결과를 `.xlsx` 파일로 내보낼 수 있습니다.
13. 행운권 추첨은 이미 당첨된 참여자를 제외하고, 관리자가 설정한 응원 메시지 가중치에 따라 메시지 개수와 총 글자수가 많은 참여자의 확률을 완만하게 높일 수 있습니다.
