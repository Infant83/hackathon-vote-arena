...
type Snapshot = {
  ...
  operationPresets?: OperationSettingsPreset[]
}

type OperationSettingsPreset = {
  id: string
  label: string
  eventId?: string
  workerName?: string
...
export class ArenaRoom {
  ...
  private operationPresets: OperationSettingsPreset[] = []
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (request.method === 'GET' && url.pathname === '/api/operation-presets') {
      return json(this.getOperationPresetManifest())
    }
    ...
    if (request.method === 'GET' && url.pathname.startsWith('/api/operation-presets/')) {
      ...
      const preset = this.operationPresets.find((entry) => entry.id === presetId)
      if (!preset) return json({ error: 'operation preset not found' }, 404)
      return json(preset.payload)
    }
  ...
  async alarm() {
    await this.loaded
    if (!this.advanceQuizPhase(Date.now())) return
  ...
  private async load() {
    ...
    this.operationPresets = this.normalizeOperationPresets(snapshot.operationPresets)
  ...
  private async persist() {
    const snapshot: Snapshot = {
      ...
      settingsVersion,
      operationPresets: this.operationPresets,
    }
  ...
  private async handleMutation(request: Request, pathname: string, body: RequestBody) {
    ...
    if (pathname === '/api/raffle') {
      ...
      const stageMatchesRule = this.raffleStage.rule === rule
      const winnerCount = this.getRaffleWinnerCount(rule, body.winnerCount ?? (stageMatchesRule ? this.raffleStage.winnerCount : undefined))
      const candidates = this.getRaffleCandidates(rule)
      const createdAt = Date.now()
      const prizeImageFile = this.getRafflePrizeImage(rule, body.prizeImageFile ?? (stageMatchesRule ? this.raffleStage.prizeImageFile : undefined))
      const prizeName = this.getRafflePrizeName(rule, body.prizeName ?? (stageMatchesRule ? this.raffleStage.prizeName : undefined))
    ...
    if (pathname === '/api/raffle/stage') {
      ...
      const prizeImageFile = this.getRafflePrizeImage(rule, body.prizeImageFile ?? (stageMatchesRule ? this.raffleStage.prizeImageFile : undefined))
      const prizeName = this.getRafflePrizeName(rule, body.prizeName ?? (stageMatchesRule ? this.raffleStage.prizeName : undefined))
      const shouldClearResult = Boolean(body.clearResult) || Boolean(this.lastRaffle && (
        rule !== this.lastRaffle.rule ||
        winnerCount !== this.lastRaffle.winnerCount ||
        prizeImageFile !== (this.lastRaffle.prizeImageFile || '') ||
        prizeName !== (this.lastRaffle.prizeName || '')
      ))
    ...
    if (pathname === '/api/quiz/confirm-mode') {
      this.activateQuizConfirmMode()
      await this.commit({ audience: true })
      return json(this.getStateForRequest(request, { slimMedia: true }))
    }
        ...
        quizInitialConfirmDelaySeconds: (() => {
          const rawValue =
            body.quizInitialConfirmDelaySeconds === undefined || body.quizInitialConfirmDelaySeconds === ''
              ? this.settings.quizInitialConfirmDelaySeconds ?? defaultQuizInitialConfirmDelaySeconds
              : Number(body.quizInitialConfirmDelaySeconds)
          return clamp(
            Math.floor(Number.isFinite(rawValue) ? rawValue : defaultQuizInitialConfirmDelaySeconds),
            0,
    ...
    if (pathname === '/api/operation-presets') {
      ...
      return json({
        ...
        ...this.getOperationPresetManifest(),
      })
    }

    if (pathname === '/api/operation-presets/delete') {
      const presetId = sanitizeOperationPresetId(body.presetId || body.id)
      const before = this.operationPresets.length
      this.operationPresets = this.operationPresets.filter((preset) => preset.id !== presetId)
      if (this.operationPresets.length === before) return json({ error: 'operation preset not found' }, 404)
  ...
  private normalizeOperationPresetPayload(input: unknown): RequestBody {
    const source = normalizeObject(input)
    const sourceTeams = Array.isArray(source.teams) ? source.teams.slice(0, 20) : []
    if (!sourceTeams.length) throw new Error('teams array required')

    const profile = normalizeEventProfile(source.event, getRuntimeEventProfile(this.eventProfile, this.roomName))
    const teams = sourceTeams.map((teamValue, index) => {
      const team = normalizeObject(teamValue)
      return normalizeTeam(
        {
          ...team,
        ...
        this.teams[index] || this.initialConfig.teams[index] || this.initialConfig.teams[0],
        index,
      )
    })
    ...
    return {
      ...
      copy: normalizeCopy({ ...this.copy, ...normalizeObject(source.copy) }),
      settings: this.getRuntimeSettings({ ...this.settings, ...normalizeObject(source.settings) }),
      teams,
      quizzes: normalizeQuizBank(source.quizzes || source.quizBank, this.quizBank),
    }
  }
  ...
  private normalizeOperationPresets(input: unknown): OperationSettingsPreset[] {
    if (!Array.isArray(input)) return []

    return input
      .map((entry): OperationSettingsPreset | null => {
        try {
          const source = normalizeObject(entry)
          const payload = this.normalizeOperationPresetPayload(source.payload)
          const label = sanitizeText(source.label, 80) || this.getDefaultOperationPresetLabel()
          const profile = normalizeEventProfile(payload.event, getRuntimeEventProfile(this.eventProfile, this.roomName))

          return {
            id: sanitizeOperationPresetId(source.id) || createOperationPresetId(label),
            label,
            ...(profile.id ? { eventId: profile.id } : {}),
            ...(profile.workerName ? { workerName: profile.workerName } : {}),
            roomName: this.roomName || 'default',
            ...(profile.settingsFile ? { settingsFile: profile.settingsFile } : {}),
            ...(sanitizeText(source.description, 160) ? { description: sanitizeText(source.description, 160) } : {}),
            createdAt: Math.max(0, Math.floor(Number(source.createdAt) || Date.now())),
            updatedAt: Math.max(0, Math.floor(Number(source.updatedAt) || Number(source.createdAt) || Date.now())),
            payload,
          } satisfies OperationSettingsPreset
        } catch {
          return null
        }
      })
      .filter((entry): entry is OperationSettingsPreset => Boolean(entry))
      .sort((left, right) => (right.updatedAt || right.createdAt) - (left.updatedAt || left.createdAt))
      .slice(0, maxOperationSettingsPresets)
  }

  private getDefaultOperationPresetLabel() {
    const profile = getRuntimeEventProfile(this.eventProfile, this.roomName)
  ...
  private operationPresetToManifestEntry(preset: OperationSettingsPreset) {
    return {
      id: preset.id,
      label: preset.label,
  ...
  private getOperationPresetManifest() {
    return {
      presets: this.operationPresets.map((preset) => this.operationPresetToManifestEntry(preset)),
    }
  }
  ...
  private getRafflePrizeImage(rule: RaffleRule, rawValue?: unknown) {
  ...
  private getRafflePrizeName(rule: RaffleRule, rawValue?: unknown) {
  ...
  private advanceQuizPhase(now = Date.now()) {
    ...
    if (this.quiz.mode === 'open' && !this.quiz.confirmModeStartedAt && this.quiz.confirmReadyAt > 0 && now >= this.quiz.confirmReadyAt) {
      this.quiz = {
        ...this.quiz,
      ...
      if (this.hasPendingCorrectQuizCandidate()) {
        changed = this.startQuizSettlement(now) || changed
      }
    }

    if (this.quiz.mode === 'settling' && this.quiz.settlementDeadlineAt > 0 && now >= this.quiz.settlementDeadlineAt) {
      changed = this.finalizeQuizSettlement(now) || changed
    }

    return changed
  }

  private async scheduleQuizAlarm() {
    const nextAt = this.getNextQuizTimerAt()
    if (!nextAt) {
  ...
  private getNextQuizTimerAt() {
    if (this.quiz.mode === 'settling' && this.quiz.settlementDeadlineAt > 0) return this.quiz.settlementDeadlineAt
    if (this.quiz.mode === 'countdown' && this.quiz.opensAt > 0) return this.quiz.opensAt
    if (
      this.quiz.mode === 'open' &&
      !this.quiz.confirmModeStartedAt &&
      this.quiz.confirmReadyAt > 0 &&
      this.hasPendingCorrectQuizCandidate()
    ) {
      return this.quiz.confirmReadyAt
    }

    return 0
  }

  private hasQuizWinnerCapacity() {
    return this.quiz.winners.length < this.quiz.winnerCount
  }

  private hasPendingCorrectQuizCandidate() {
    if (!this.hasQuizWinnerCapacity()) return false

    const existingParticipantIds = new Set(this.quiz.winners.map((winner) => winner.participantId))
    return this.quiz.answers.some(
      (answer) => answer.correct && !answer.rank && !existingParticipantIds.has(answer.participantId),
    )
  }

  private isQuizConfirmModeActive(now = Date.now()) {
    return Boolean(this.quiz.confirmModeStartedAt) || Boolean(this.quiz.confirmReadyAt && now >= this.quiz.confirmReadyAt)
  }

  private startQuizSettlement(now = Date.now()) {
    if (this.quiz.mode !== 'open' || !this.hasPendingCorrectQuizCandidate()) return false

    this.quiz = {
      ...this.quiz,
      mode: 'settling',
      confirmModeStartedAt: this.quiz.confirmModeStartedAt || now,
  ...
  private activateQuizConfirmMode(now = Date.now()) {
    this.advanceQuizPhase(now)
    if (this.quiz.mode !== 'countdown' && this.quiz.mode !== 'open' && this.quiz.mode !== 'settling') return false

    let changed = false
    if (!this.quiz.confirmModeStartedAt) {
      this.quiz = {
        ...this.quiz,
        confirmModeStartedAt: now,
        updatedAt: now,
    ...
    if (this.quiz.mode === 'open' && this.hasPendingCorrectQuizCandidate()) {
      changed = this.startQuizSettlement(now) || changed
    }
  ...
  private openQuiz(body: RequestBody) {
    ...
    this.quiz = {
      ...
      opensAt,
      confirmReadyAt: opensAt + this.getQuizInitialConfirmDelayMs(),
      confirmModeStartedAt: 0,
      settlementStartedAt: 0,
      settlementDeadlineAt: 0,
      createdAt: now,
  ...
  private submitQuizAnswer(person: Participant | null, textValue: unknown, body: RequestBody = {}) {
    ...
    if (correct && this.quiz.mode === 'open' && this.hasQuizWinnerCapacity() && !alreadyWinner) {
      if (this.isQuizConfirmModeActive(serverReceivedAt)) {
        if (!this.quiz.confirmModeStartedAt) {
          this.quiz = {
            ...this.quiz,
        ...
        this.startQuizSettlement(serverReceivedAt)
      }
    }
  ...
  private getQuizInitialConfirmDelayMs() {
    return this.getRuntimeSettings().quizInitialConfirmDelaySeconds * 1000
  }
...
function sanitizeOperationPresetId(value: unknown) {
  return String(value || '')
    .replace(/[^a-zA-Z0-9_.:-]/g, '')
    .slice(0, 96)
}

function createOperationPresetId(label: string) {
...
function isAdminProtectedRequest(url: URL, method: string) {
  ...
  if (method === 'GET' && (url.pathname === '/api/operation-presets' || url.pathname.startsWith('/api/operation-presets/'))) {
    return true
  }
...
function normalizeRaffleStage(value: unknown): RaffleStageState {
  ...
  const winnerCount = source.winnerCount === undefined ? undefined : clamp(Math.floor(Number(source.winnerCount) || 1), 1, 10)
  return {
    ...
    prizeImageFile: sanitizeLogoPath(source.prizeImageFile),
    prizeName: sanitizeText(source.prizeName, 80),
...
function normalizeQuizState(value: unknown): QuizState {
  ...
  return {
    ...
    confirmReadyAt: Math.max(0, Number(source.confirmReadyAt) || 0),
    confirmModeStartedAt: Math.max(0, Number(source.confirmModeStartedAt) || 0),
